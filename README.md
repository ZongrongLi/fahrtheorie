...
学习进度存浏览器本地；账号、AI 额度、支付走 Cloudflare Worker 后端。线上：https://fahrtheorie.homes（当前 build v92）。

- 题库：官方目录（已更新至 2026-04-01 最新版），默认只显示 **B 照 1262 题**，可切换「全部车型 2407 题」
...
```
浏览器
 ├── data/questions.js      题库 · **B 照 1262 题**（德语原文 + 英文 + 正确答案 + 官方解释 + 图片/视频索引 + 主语）
 ├── data/questions-more.js 其余 1145 题（卡车/公交等），切到「全部车型」才按需拉
 ├── data/zh.js             中文译文（题干 / 选项 / 官方解释 / 主语）+ 主题章节中文名，刷题语言含中文才按需拉
 ├── data/videos.js         251 道视频题的官方视频路径
 ├── i18n.js                英文界面包（默认语言，也是所有语言包的兜底）
 ├── i18n-more.js           其余 9 种界面包，界面语言不是英语才按需拉
